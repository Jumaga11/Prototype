...
<div class="max-w-md sm:max-w-2xl md:max-w-4xl mx-auto p-6 rounded-lg shadow-lg bg-gradient-to-br from-green-200 via-purple-200 to-pink-200">
  <h3 class="text-xl font-bold text-black mb-2">Número de Factura: <span class="font-semibold text-gray-600">${idFactura}</span></h3>
  <h3 class="text-xl font-bold text-black mb-2">Nombre del Usuario: <span class="font-semibold text-gray-600">${nombreUsuario}</span></h3>
  <h3 class="text-xl font-bold text-black mb-2">Nombre del Cliente: <span class="font-semibold text-gray-600">${nombreCliente}</span></h3>
</div>
      ...
      .then((detalle_facturas) => {
        ...
        if (facturasFiltradas.length === 0) {
          ...
          facturasFiltradas.forEach((detalle_factura) => {
            ...
            facturaCard.className = "p-4 border rounded-lg shadow-md bg-white flex";
          
            facturaCard.innerHTML = `
              <div class="flex-1">
                <p class="text-justify"><strong>Nombre Producto:</strong> ${detalle_factura.nombre}</p>
                <p class="text-justify"><strong>Cantidad:</strong> ${detalle_factura.cantidad}</p>
                <p class="text-justify"><strong>Precio:</strong> ${detalle_factura.precio_unitario}</p>
                <p class="text-justify"><strong>Subtotal:</strong> ${subtotal.toFixed(2)}</p>
              </div>
              
              <div class="flex flex-col items-center justify-center gap-4">
                <button onclick="editDetalleFactura(${detalle_factura.id_detalle})" class="hover:text-blue-500 focus:outline-none">
                  <img src="img/edit.svg" alt="edit" class="w-6 h-6">
                </button>
                <button onclick="confirmDelete(${detalle_factura.id_detalle})" class="hover:text-red-500 focus:outline-none">
                  <img src="img/delete.svg" alt="delete" class="w-6 h-6">
                </button>
              </div>
            `;
